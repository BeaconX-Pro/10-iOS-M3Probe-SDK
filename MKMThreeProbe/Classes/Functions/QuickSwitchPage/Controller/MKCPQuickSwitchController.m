...
@interface MKCPQuickSwitchController ()<UICollectionViewDataSource,
...
@property (nonatomic, strong)MKCPQuickSwitchModel *dataModel;

@end

@implementation MKCPQuickSwitchController

- (void)dealloc {
    NSLog(@"MKCPQuickSwitchController销毁");
}
...
- (void)setConnectStatusToDevice:(BOOL)connect{
    [[MKHudManager share] showHUDWithTitle:@"Setting..."
                                     inView:self.view
                              isPenetration:NO];
    [MKCPInterface cp_configConnectStatus:connect sucBlock:^(id returnData) {
...
- (void)setButtonPowerOffToDevice:(BOOL)isOn {
    [[MKHudManager share] showHUDWithTitle:@"Setting..."
                                     inView:self.view
                              isPenetration:NO];
    [MKCPInterface cp_configButtonPowerStatus:isOn sucBlock:^(id returnData) {
        [[MKHudManager share] hide];
...
- (void)commandForLockState:(BOOL)isOn{
    ...
    [MKCPInterface cp_configLockState:(isOn ? mk_cp_lockStateOpen : mk_cp_lockStateUnlockAutoMaticRelockDisabled) sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        MKBXQuickSwitchCellModel *cellModel = self.dataList[2];
        cellModel.isOn = isOn;
        [MKCPConnectManager shared].passwordVerification = isOn;
        [self.view showCentralToast:@"Success!"];
...
- (void)setButtonResetToDevice:(BOOL)isOn {
    [[MKHudManager share] showHUDWithTitle:@"Setting..."
                                     inView:self.view
                              isPenetration:NO];
    [MKCPInterface cp_configResetBeaconByButtonStatus:isOn sucBlock:^(id returnData) {
        [[MKHudManager share] hide];
...
- (void)configTriggerLEDNotification:(BOOL)isOn {
    [[MKHudManager share] showHUDWithTitle:@"Setting..."
                                     inView:self.view
                              isPenetration:NO];
    [MKCPInterface cp_configLEDTriggerStatus:isOn sucBlock:^(id returnData) {
        [[MKHudManager share] hide];
...
- (MKCPQuickSwitchModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCPQuickSwitchModel alloc] init];
    }
    return _dataModel;
}
