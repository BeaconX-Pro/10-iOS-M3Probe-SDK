...
@interface MKCPQuickSwitchModel ()
...
@implementation MKCPQuickSwitchModel
...
- (void)readWithSucBlock:(void (^)(void))sucBlock failedBlock:(void (^)(NSError *error))failedBlock {
    dispatch_async(self.readQueue, ^{
        ...
        if (![self readTriggerLED]) {
            [self operationFailedBlockWithMsg:@"Read Trigger LED Error" block:failedBlock];
            return;
        }
        ...
        self.passwordVerification = [MKCPConnectManager shared].passwordVerification;
...
- (BOOL)readConnectable {
    ...
    [MKCPInterface cp_readConnectEnableStatusWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)readTriggerLED {
    __block BOOL success = NO;
    [MKCPInterface cp_readLEDTriggerStatusWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)readTurnOffByButton {
    __block BOOL success = NO;
    [MKCPInterface cp_readButtonPowerStatusWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)readResetByButton {
    __block BOOL success = NO;
    [MKCPInterface cp_readResetBeaconByButtonStatusWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
