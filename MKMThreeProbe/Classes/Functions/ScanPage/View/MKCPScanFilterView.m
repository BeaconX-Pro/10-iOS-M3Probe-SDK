...
@interface MKCPScanFilterView ()<UIGestureRecognizerDelegate>
...
@property (nonatomic, copy)void (^searchBlock)(NSString *deviceName, NSInteger searchRssi);

@end

@implementation MKCPScanFilterView

- (void)dealloc{
    NSLog(@"MKCPScanFilterView销毁");
}
...
+ (void)showSearchDeviceName:(NSString *)deviceName
    ...
    MKCPScanFilterView *view = [[MKCPScanFilterView alloc] init];
    [view showSearchDeviceName:deviceName rssi:rssi searchBlock:searchBlock];
}
...
- (void)showSearchDeviceName:(NSString *)deviceName
    ...
    self.nameTextField.text = SafeStr(deviceName);
...
- (UIImageView *)signalIcon{
    if (!_signalIcon) {
        _signalIcon = [[UIImageView alloc] initWithFrame:CGRectMake(offset_X,
                                                                    ...
                                                                    signalIconWidth,
                                                                    signalIconHeight)];
        _signalIcon.image = LOADICON(@"MKMThreeProbe", @"MKCPScanFilterView", @"cp_wifiSignalIcon.png");
    }
    return _signalIcon;
}
...
- (UIImageView *)graySignalIcon {
    if (!_graySignalIcon) {
        _graySignalIcon = [[UIImageView alloc] initWithFrame:CGRectMake(backViewWidth - offset_X - signalIconWidth,
                                                                    ...
                                                                    signalIconWidth,
                                                                    signalIconHeight)];
        _graySignalIcon.image = LOADICON(@"MKMThreeProbe", @"MKCPScanFilterView", @"cp_wifiGraySignalIcon.png");
    }
