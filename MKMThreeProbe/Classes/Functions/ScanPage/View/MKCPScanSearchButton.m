...
@implementation MKCPScanSearchButtonModel
@end

@interface MKCPScanSearchButton ()
...
@implementation MKCPScanSearchButton
...
- (void)clearButtonPressed{
    ...
    if ([self.delegate respondsToSelector:@selector(cp_scanSearchButtonClearMethod)]) {
        [self.delegate cp_scanSearchButtonClearMethod];
    }
}

- (void)searchButtonPressed{
    if ([self.delegate respondsToSelector:@selector(cp_scanSearchButtonMethod)]) {
        [self.delegate cp_scanSearchButtonMethod];
    }
}

#pragma mark - setting
- (void)setDataModel:(MKCPScanSearchButtonModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCPScanSearchButtonModel.class]) {
        return;
    }
    self.titleLabel.text = (ValidStr(_dataModel.placeholder) ? _dataModel.placeholder : @"Edit Filter");
    NSMutableArray *conditions = [NSMutableArray array];
    if (ValidStr(_dataModel.searchCondition)) {
        [conditions addObject:_dataModel.searchCondition];
    }
...
- (UIImageView *)searchIcon{
    if (!_searchIcon) {
        ...
        _searchIcon.image = LOADICON(@"MKMThreeProbe", @"MKCPScanSearchButton", @"cp_searchGrayIcon.png");
    }
    return _searchIcon;
}
...
- (UIButton *)clearButton{
    if (!_clearButton) {
        ...
        [_clearButton setImage:LOADICON(@"MKMThreeProbe", @"MKCPScanSearchButton", @"cp_clearButtonIcon.png") forState:UIControlStateNormal];
